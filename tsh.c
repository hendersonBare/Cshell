...
void eval(char *cmdline) 
{
  ...
  char *builtin_commands[] = {"quit", "jobs", "bg", "fg"};
  ...
  for (int i = 0; i < 4; i++) {
    if (!strcmp(argv1[0], builtin_commands[i])) { //check if argv1[0] is the right place to make this comparison
      builtin_cmd(argv1);
      return;
    }
  }
  //this is the case in which we only have 1 command, which is an executable
  //we want to fork a child process and load that child process, we will start with this and 
  //worry about all the other flags and commands later, we will start with basic implementation then move
  //to fg and bg functionality

  //TODO: find out how to block SIGCHLD before forking the child, then unblock after (may also need to block SIGINT, SIGTSTP)
  //this is needed to avoid a race condition in which the child is reaped by the sigchld handler before parent can add it to jobs list
  sigset_t sigs;
  sigemptyset(&sigs);
  ...
  pipe(fd);
  int pid;
  if ((pid = fork()) == 0) { //here we are forking the child
    setpgid(0,0); //puts it in a new group so that it is not a fg process in the context of the unix shell
    //printf("child unblocked signals");
    close(fd[0]);
	  close(fd[1]);
  ...
  //if bg != 0 then we init w/ 1 else we init w 2
  int initstate = BG;
  if (bg == 0) {
    initstate = FG;
  }
  addjob(jobs,pid,initstate,cmdline);
  //printf("parent unblocked signals");
  sigprocmask(SIG_UNBLOCK, &sigs, NULL);
  close(fd[0]);
	close(fd[1]);
...
void do_bg(int jid) 
{
  //TODO: find the job from the jid, if it doesnt exist print a message
...
void waitfg(pid_t pid)
{
  ...
  while(job && (job->state == FG)) {
    sigprocmask(SIG_BLOCK, &sigs, NULL);
    sigsuspend(&sigs);
  } //do nothing while the job is in the FG
  return;
}
...
void sigchld_handler(int sig) 
{
  ...
  while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
    ...
    if (WIFSTOPPED(status)) {
      ...
      //sio_puts("child terminanted so we removed the child\n");
      deletejob(jobs, pid);
    }
    
    else if (WIFSIGNALED(status)) {
      //sio_puts("a signal was received so we removed the child\n");
      sio_puts("Job [");
      sio_putl(job->jid);
      sio_puts("] (");
      sio_putl(pid);
...
void sigint_handler(int sig) 
{
  //sio_puts("sigint signal received");
  pid_t pid = 0;
...
void sigtstp_handler(int sig) 
{
  //sio_puts("sigtstp signal received \n");
  pid_t pid = 0;
  for(int i = 0; i < MAXJOBS; i++) {
